...
const serviceRoleKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InV4cXliZHF2dHVqbGNraGxqcHBoIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc3NjEyMDI3MSwiZXhwIjoyMDkxNjk2MjcxfQ.TSd6lN9QQVI8dJ_sSo8JvO7LEjjDfrPk15veDAXsqOo';
...
CREATE POLICY os_features_access ON os_features FOR ALL TO authenticated USING (true) WITH CHECK (true);
...
CREATE POLICY os_analytics_insights_access ON os_analytics_insights
    ...
    USING (EXISTS (SELECT 1 FROM store_members WHERE store_id = os_analytics_insights.store_id AND user_id = auth.uid()))
    WITH CHECK (EXISTS (SELECT 1 FROM store_members WHERE store_id = os_analytics_insights.store_id AND user_id = auth.uid()));
`;

async function runMigration() {
  console.log('Creating OS tables...');
